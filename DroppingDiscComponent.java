...
public class DroppingDiscComponent extends JComponent {
    ...
    public void setThread (MyThread th) {
        t = th;
    }
    public void setThreadLeftMost (MyThreadLeftMost th) {
        tl = th;
    }

    public void setLeftMostCoordinates(int height, int width, int column) {
        isLeftMost = true;
        restingColumn = column;
        int squareDimension = min(width/9, height/8);
        int circleDiameter = 3*squareDimension/4;
        discBottomY = 7 * (getHeight() / 2 - 3 * squareDimension) / 8;
        discCenterX = width/2 - 3*squareDimension + restingColumn*squareDimension;
